...
for epoch in range(num_epochs):
    ...
    with torch.no_grad():
        for batch in val_loader:
            ...
            val_accuracy = val_correct.float() / torch.tensor(val_total, dtype=torch.float)


    train_loss /= len(train_loader)
    val_loss /= len(val_loader)

    print(f"Epoch {epoch+1}:  loss: {train_loss:.4f} val_loss: {val_loss:.4f} val_accuracy: {val_accuracy:.4f}")
